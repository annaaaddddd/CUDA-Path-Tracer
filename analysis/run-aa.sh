...
# Writes analysis/aa-timing.csv and restores src/pathtrace.cu on exit.
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "config,sort,aa,ms_per_iter,fps" > analysis/aa-timing.csv
...
run() {  # name sort aa
    ...
    awk -v n="$1" -v s="$2" -v a="$3" '/ms\/iteration/ {
        gsub(/[()]/,""); print n","s","a","$5","$7 >> "analysis/aa-timing.csv" }' "$log"
}

run aa-only 0 1
run all-on  1 1

echo "wrote analysis/aa-timing.csv"
